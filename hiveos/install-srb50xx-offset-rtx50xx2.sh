...
EXCLUDED_RUNNING_UUID="GPU-52d71608-bd1f-6240-0ef3-78eef87230bd"
...
apply_offset_to_50xx() {
  ...
  nvidia-smi --query-gpu=index,uuid,name --format=csv,noheader | while IFS=',' read -r idx uuid name; do
    idx="$(echo "$idx" | tr -d ' ')"
    uuid="$(echo "$uuid" | tr -d ' ')"
    name="$(echo "$name" | sed 's/^ *//')"

    if is_50xx_gpu "$name"; then
      if [ "$offset_value" = "$RUNNING_OFFSET" ] && [ "$uuid" = "$EXCLUDED_RUNNING_UUID" ]; then
        log "GPU ${idx} ${name} ${uuid} kept at +${START_OFFSET} (excluded from +${RUNNING_OFFSET})"
        continue
      fi

      log "GPU ${idx} ${name} ${uuid} -> +${offset_value}"
      set_core_offset "$idx" "$offset_value"
    else
      log "GPU ${idx} ${name} ${uuid} skipped"
    fi
  done
}
...
[Unit]
Description=SRBMiner RTX 50xx offset switch for RTX-50XX2
